	...
	//metodo estatico, al cual le pasamos un objeto de la clase Request 
 	...
 	public static function run(Request $peticion){
		if(!$peticion->getMicroservice()){
			...
			$controller = ucwords($peticion->getControlador()) . "Controller"; //obtenemos el controlador y le concatenamos Controller al final, para poder buscarlo luego
			$microservice = $peticion->getMicroservice();
			
			$rutaControlador = ROOT . "modules" . DS . $microservice . DS . "controllers" . DS . $controller . ".php"; //armamos la ruta hasta el controlador que se solicita
			$metodo = $peticion->getMetodo();
			$args = $peticion->getArgs();
		}
		...
		//var_dump($rutaControlador);die();
 		if(is_readable($rutaControlador)){
			...
			header("location: index.php?url=error/access/error/404");
